...
for (var i = 0; i < 9; i++) {
  casas[i].addEventListener('click', (event) => {
    ...
    if ((event.target.value == '_') && (vencedor == '_')) {
      ...
      vencedor = vitoria(); //Executa a função vitoria() para verificar se há um vencedor

      //se o vencedor existe, imprime
      if (vencedor != '_') {
        label_jogador.innerText = "Vencedor: " + vencedor;
...
var sortearJogador = function () {
  if (Math.floor(Math.random() * 2) == 0) {
    ...
    label_jogador.style.color = 'black'; //deixa o texto na cor preta
  } else {
...
//Altera a vez entre os jogadores X e O
